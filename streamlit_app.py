...
st.info("このサイトでは、３ブロック課題の提出ファイルが最低限の体裁を持っているかチェックします。ここでは３ブロック課題の**提出はできません。**")
...
if uploaded_file:
    ...
    if res:
        ...
        # スコア表示
        if score == 12:
            st.success(f"🎉 最低限の体裁チェックはOKですが、提出の際は、３ブロック課題チェックリストの**Doneの条件をよく読み自分でチェック**してください。")
            st.balloons()
        elif score >= 8:
        ...
        if user_id:
            ...
            for r in res:
                ...
                table_html += f"""<tr>
                        <td style="border: 1px solid #ddd; padding: 1px; text-align: center;">{r['No']}</td>
                        <td style="border: 1px solid #ddd; padding: 1px;">{r['チェック項目']}</td>
                        <td style="border: 1px solid #ddd; padding: 1px; text-align: center; font-weight: bold; color: {color};">{r['判定']}</td>
                        <td style="border: 1px solid #ddd; padding: 1px; font-family: monospace;">{r['内容/数式']}</td>
                    </tr>"""
